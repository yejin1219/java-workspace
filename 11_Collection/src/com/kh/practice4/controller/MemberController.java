...
public class MemberController {
	...
	public boolean changePassword(String id, String oldPw, String newPw) {
		...
		if(map.containsKey(id) && map.get(id).getPassword().equals(oldPw)) {
			 map.get(id).setPassword(newPw);  // --> map.put(id, new Member(newPw, map.get(id).getName())) 로 해도 됨 
			 return true;
		}
		return false;
	}
	...
	public void changeName(String id, String newName) {
		 ...
		 * 
		 * */
		map.get(id).setName(newName); // map.put(id, new Member( map.get(id).getPassword(), newName()))
